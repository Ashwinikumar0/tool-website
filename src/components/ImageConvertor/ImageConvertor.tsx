...
const ImageConverter = ({ inputFormat, outputFormat }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file.type !== `image/${inputFormat}`) {
      setError(
        `Selected file does not match the expected input format: image/${inputFormat}`,
      );
      return;
    }
          ...
          canvas.toBlob(
            (blob) => {
              resolve(
                new File(
                  [blob],
                  file.name.replace(/\.\w+$/, `.${outputFormat}`),
  ...
  return (
    <Box component={"section"}>
      <Typography variant="h3" component={"h3"} align="center" pt={2}>
        <Typography component={"span"} variant="h3" color={"orange"}>
          {inputFormat.toUpperCase()}
        </Typography>{" "}
        to{" "}
        <Typography component={"span"} variant="h3" color={"green"}>
      ...
      >
        ...
        Converter. ConvertMaster converts your image files online. Best way to
        convert your{" "}
        <Typography component={"span"} color={"orange"}>
          {inputFormat.toUpperCase()}
        </Typography>{" "}
          ...
          sx={{
            ...
            border:
              theme.palette.mode === "light"
                ? "1px solid rgba(164, 164, 164, 0.13)"
                : "1px solid #FFF",
            borderRadius: "35px",
            boxShadow:
              theme.palette.mode === "light"
                ? `rgb(241, 241, 241) 1px 13px 12px 0px`
                : "#000",
            width: { xs: "90%", md: "100%" },
          }}
        >
      ...
      {selectedFile && (
        ...
        >
          ...
          >
            <Typography variant="subtitle2" pt={1} component={"div"}>
              Original Image
            </Typography>
            {selectedFile && (
              <div>
          ...
          >
            ...
            {convertedFile && (
              <div>
                <Typography variant="caption" component={"div"}>
                  File Name: {convertedFile.name}
                </Typography>
                <Typography variant="caption" component={"div"}>
                  File Size: {(convertedFile.size / 1024 / 1024).toFixed(2)} MB
                </Typography>
                <Box
                  sx={{
                    overflow: "hidden",
      ...
      <Box sx={{ pt: 5, margin: "auto" }}>
        ...
        >
          Transform your images in just a few clicks with our user-friendly PNG
          to WEBP converter. Whether you need to convert a single photo or a
          batch of images, our tool streamlines the process, making it
          hassle-free and efficient. With over a hundred customization options,
          you can tailor your images to meet your exact needs. Say goodbye to
          tedious manual conversions and embrace seamless image transformation
          with our online converter.
        </Typography>
        <Typography variant="h6" pt={1}>
          How to Convert {inputFormat.toUpperCase()} to{" "}
          {outputFormat.toUpperCase()}
        </Typography>
        <List
